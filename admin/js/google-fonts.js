...
function prepareToggleSelectAllVariants() {
	const selectAllVariantsElement = document.getElementById(
		'select-all-variants'
	);
...
function toggleSelectAllVariants() {
	const variantCheckboxes = document.querySelectorAll(
		'#font-options input[type="checkbox"]'
	);
...
async function get_google_fonts() {
	const currentUrl = new URL(
		document.getElementById('google-fonts-script-js').src
	);
	const fallbackURL =
		currentUrl.origin +
		currentUrl.pathname.replace(
			'admin/js/google-fonts.js',
...
function displayFontOptions() {
	...
	for (const variant of fontSelected.variants) {
		...
		const variantUrl = fontSelected.files[variant].replace(
			'http://',
			'https://'
		);
...
function onFontVariantChange() {
	...
	const googleFontsSelectedElement = document.getElementById(
		'google-font-variants'
	);
	googleFontsSelectedElement.value = Object.keys(variantsSelected)
		.map((key) => `${key}::${variantsSelected[key]}`)
		.join(',');
...
function checkIfFormIsAbleToSubmit() {
	...
	submitElement.disabled = !Array.from(variantCheckboxes).find(
