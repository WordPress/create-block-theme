...
function SelectedVariantsOutline ( { selectionData, removeVariant } ) {
    ...
    const flatSelectionData = Object.keys( selectionData ).map( ( family ) => {
    ...
    const variantsCount = Object.keys( selectionData ).reduce( ( acc, family ) => {
    ...
    const totalSize = bytesToSize( flatSelectionData.reduce( ( acc, face ) => {
    ...
    return (
        <div className="variants-outline">
                    ...
                    <div className="variants-list">
                        { Object.keys(selectionData).map( ( key, i ) => (
                            <div className="variants-family">
                                ...
                                { selectionData[key].faces.map( ( face, i ) => (
                                    <div className="variant-row" key={ `selected-variant-${i}` }>
                                        <div>{face.weight} {face.style}</div>
                                        <div>{ getFileSize( face.src ) }</div>
                                        <div>
                                            <Button onClick={
                                                () => removeVariant ( selectionData[key].family, face.weight, face.style )
                                            } icon={ trash } iconSize={ 15 } isSmall />
                                        </div>
                                    </div>
                                ))}
                            </div>
            ...
            <div className="variants-total">
                <div>{ variantsCount } { _n( "Variant", "Variants",  variantsCount, "create-block-theme" ) }</div>
                <div>{ totalSize }</div>
            </div>
